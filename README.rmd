...
::: callout-tip
An [`llms.txt`](https://r.esri.com/calcite/llms.txt) file is available to provide context for LLMs when working with this package.
:::
...
Use `calcite_scrim()` to overlay a loading indicator on a section of content.
Set `loading = TRUE` to show a centered busy indicator. Wrap the target area
...
htmltools::div(
  ...
  # ... your content ...
  calcite_scrim(id = "my_scrim", loading = FALSE)
)
```

Toggle from the server with `update_calcite()`:

```{r, eval = FALSE}
update_calcite("my_scrim", loading = TRUE)
# ... do work ...
update_calcite("my_scrim", loading = FALSE)
```
